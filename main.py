...
def save_packages_to_file(packages, folder_path):
    """Saves the list of packages to a file and creates a .bat file for installation."""
    ...
    try:
        ...
        # Create the .bat file for installation
        bat_file_path = os.path.join(folder_path, "install_packages.bat")
        with open(bat_file_path, 'w', encoding='utf-8') as bat_file:
            bat_file.write("@echo off\n")
            bat_file.write('for /f "delims=" %%p in (packages.txt) do (\n')
            bat_file.write('    echo Installing package: %%p\n')
            bat_file.write('    npm install %%p\n')
            bat_file.write(')\n')
        print(f"Installation batch file saved to: {bat_file_path}")
